...
(async()=>{
	...
	let META_PATH = path.resolve(__dirname, 'meta.json');

	if ( ARGV.length > 0 ) {
		META_PATH = path.resolve(process.cwd(), ARGV[0]);
	}
	

	process.chdir(path.dirname(META_PATH));
	const meta_str = fs.readFileSync(META_PATH, 'utf-8');
